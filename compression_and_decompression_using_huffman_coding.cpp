...
void buildHuffmanTree(string txt)
{
    ...
    // Traversing the original string to compute the frequencies map 
    for(auto it:txt)
    {
        frequencies[it]++;
    }
    ...
    {
        //Removing the two nodes with highest priority i.e least freq from the heap.
    ...
    //creating the encoded string for the original string by traversing through the original string and HuffmanCodes map.
    string encodedtxt="";
    for(auto it:txt)
    {
